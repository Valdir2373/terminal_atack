...
import { isEmailDuplicate } from "../../utils/isEmailDuplicate";
import { buscarEmailsPorKeywords } from "../../reader";

export class Emails {
  private validFile: string;
  constructor() {
    this.validFile = "emails_validos.txt";
  }
  async selectOption() {
    return await select({
  ...
  async terminal() {
    ...
    await this.managerOption(option);
  }
  private async managerOption(option: string) {
    const validFile = "emails_validos.txt";
    const emailAction = option;
    ...
    switch (option) {
      ...
      case "clean":
        ...
        break;
      case "auto_collect":
        await this.auto_collect();
        break;
      case "massive_validation":
        await this.massive_validation();
        break;
      case "delete":
  ...
  private async massive_validation() {
    ...
    const existingEmails = new Set(
      fs.existsSync(this.validFile)
        ? fs
            .readFileSync(this.validFile, "utf-8")
            .split("\n")
    ...
    for (const block of blocks) {
      ...
      if (emailMatch && passMatch) {
        ...
        try {
          ...
          fs.appendFileSync(this.validFile, `${email}:${cleanPass}\n`);
